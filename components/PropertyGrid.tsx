            ...
            <div className="flex flex-col gap-6">
                {properties.map((prop) => (
                    <div key={prop.id} className="group bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col md:flex-row shadow-sm">
                        {/* Image Side */}
                        <div className="md:w-2/5 aspect-3/2 md:aspect-auto overflow-hidden relative shrink-0">
                            <Image
                                src={prop.images[0]}
                                alt={prop.title}
                                fill
                            ...
                            <div className="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors" />
                                <div className="absolute top-4 left-4 flex flex-col gap-2">
                                    <span className={`px-3.5 py-1.5 rounded-full text-[13px] font-bold shadow-md ${prop.listingType === "For Sale" ? "bg-blue-100/90 text-[#1E3A8A] backdrop-blur-md" : "bg-orange-500 text-white"}`}>
                                        {prop.listingType}
                                    </span>
                                </div>
                        ...
                        <div className="p-6 md:p-8 flex-1 flex flex-col justify-center">
                            ...
                            <div className="mt-auto flex gap-4">
                                ...
                                <a 
                                    ...
                                    className="flex-1 bg-white border md:flex-none md:w-48 border-[#1e3a8a] hover:bg-blue-50 text-[#1e3a8a] font-medium py-3 px-4 rounded-md transition-colors flex items-center justify-center gap-2 shadow-sm text-[15px]"
                                >
            ...
            {properties.map((prop) => (
                <div key={prop.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 group border border-slate-200 flex flex-col relative">
                    <div className="relative aspect-3/2 w-full overflow-hidden">
                        ...
                        />
                        <div className="absolute top-4 left-4 flex flex-col gap-2">
                            <span className={`px-3.5 py-1.5 rounded-full text-[12px] font-bold shadow-md ${prop.listingType === "For Sale" ? "bg-blue-100/90 text-[#1E3A8A] backdrop-blur-md" : "bg-orange-500 text-white"}`}>
                                {prop.listingType}
                            </span>
                        </div>
                    </div>
                    <div className="p-7 flex-1 flex flex-col">
                        ...
                        <div className="mt-auto flex gap-4">
                            ...
                            <a 
                                target="_blank" 
                                href={`https://wa.me/918426022000?text=Hi, I'm interested in the property: "${prop.title}" - ${typeof window !== 'undefined' ? window.location.origin : ''}/properties/${prop.slug}`}
                                className="flex-1 bg-white border border-[#1e3a8a] hover:bg-blue-50 text-[#1e3a8a] font-medium py-3 px-2 rounded-md transition-colors flex items-center justify-center gap-1.5 shadow-sm text-[14px] lg:text-[15px] whitespace-nowrap"
                            >
                                <MessageSquareCode className="w-4 h-4 shrink-0" /> WhatsApp
                            </a>
                        </div>
                    </div>
                </div>
